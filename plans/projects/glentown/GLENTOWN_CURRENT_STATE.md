...
- **Tourism Browse Mode:** NEXT BOUNDED APP PASS. Topbar is `Back | Tourism (Browse Mode) | Mode Toggle`; Browse banner remains visible; Browse search remains ordinary search and never implicitly starts Trip Planner. Town uses `All | Places | Experiences | Stays | Deals`; Province/National uses `All | Destinations | Experiences | Stays | Deals`. Deals secondary filters and context-sensitive Sort are approved. Plan Mode implementation remains deferred.
- **Tourism Plan Mode / Trip Planner:** DECIDED / PLANNED. Canonical detail is `GLENTOWN_TOURISM_TRIP_PLANNER.md`: Planning Destination distinct from global scope; natural-language intent plus inclusive dates, Adults/Kids, Rooms/Budget; conditional ordered multi-destination Step 0; Experience multi-select; Stay/room; optional Food Stops; start/end Airport or Local Address; flight context; inclusion-aware transport-gap resolution; informational Commute Options until Riders; conditional Vehicle Rentals (car minimum 8h, motorcycle minimum 24h); and Free Time recommendations after hard commitments.
- **Geographic discovery:** Town/Province/National Place eligibility now uses descendant-aware API scope semantics; Puerto Princesa Place records remain eligible under Palawan and Philippines scopes. This closes only the geographic-boundary portion of T1.3.8; universal Search/ranking remains open.
- **Tourism Place media:** T1.3.13 is functionally demonstrated on Samsung at Puerto Princesa scope: Baywalk Demo Events Pavilion and Harborlight Demo Cafe render authoritative `featured_media`; other Place cards remain cleanly text-only when media is null. Destination hero media remains a separate T1.3.9 concern.
- **Home map:** T1.3.10 API half now has a privacy-safe `/api/v1/map-listings` projection for map-public Properties and Events using descendant-aware scope. Home currently has no embedded map before Around You and no demo map-visible fixtures. Additional categories remain excluded until public-location semantics exist; viewport bounds are still absent. Canonical marker design is GlenTown pin silhouette + Explore category icon + Explore semantic color; no fabricated/default fallback coordinates.
- **Home media quality:** Community/service media and identity/privacy contracts are clean in the latest audit, but Food demo imagery still has semantic quality debt: Grilled Liempo uses grilled-chicken imagery, Kare-Kare uses generic vegetable-stew imagery, and Chicken Inasal shares the same chicken image. Do not call the demo media set visually final until corrected.
