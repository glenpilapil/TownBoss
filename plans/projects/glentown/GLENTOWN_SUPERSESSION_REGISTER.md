...
| GlenTown Beta manual credit top-up + proof + admin approval | Context-aware Pay-At plus licensed replaceable provider architecture; purchased Credits/manual proof deferred. | GlenTown Decisions, 2026-09-24. | Historical ledger architecture remains preserved but is not active GlenTown Beta payment direction. |
| `I Need...` / `Requests Lite` as the customer-facing demand/request label | `Looking For` is the canonical resident-facing demand expression; underlying demand/request records and lifecycle primitives may remain internal implementation details. | `GLENTOWN_DECISIONS_AND_RULES.md` rules 32-33; `GLENTOWN_WEB_WINDOWS_VISUAL_AUTHORITY.md`. | Do not reintroduce `I Need...` or `Requests Lite` as normal customer-facing creation labels. Route Looking For needs across relevant domains without forcing the resident to know the internal domain first. |
